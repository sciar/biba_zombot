namespace BibaFramework.BibaGame
{
    public class BibaPointEvents
    {
        public const string None = "None";
        public const string first_start = "first_start";
        public const string first_equip_input = "first_equip_input";
        public const string first_round_completed = "first_round_completed";
        public const string first_game_completed = "first_game_completed";
        public const string round_completed = "round_completed";
        public const string game_completed = "game_completed";
        public const string first_successful_scan = "first_successful_scan";
        public const string scan_completed = "scan_completed";
        public const string successfully_paired = "successfully_paired";
    }
}
